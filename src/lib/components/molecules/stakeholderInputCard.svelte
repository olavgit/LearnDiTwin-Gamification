...
<script lang="ts">
	onMount(async () => {
		...
		maatregelenStore.set(
			maatregelenSnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() })) as Maatregelen[]
		);
	...
	const calculateVrijwilligers = (activeValue: number, maatregel: any) => {
		if (activeValue === 0) {
			...
			const value =
				Number(maatregel.minimaal) +
				((activeValue - 1) * (Number(maatregel.maximaal) - Number(maatregel.minimaal))) / 9;
